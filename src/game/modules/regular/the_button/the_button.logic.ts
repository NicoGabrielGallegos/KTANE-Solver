...
import { totalBatteries } from "../../../widgets/batteries/batteries.utils";
...
export function solveButton(button: Button, edgework: BombEdgework): ButtonSolution {
    ...
    if (isRed(button) && saysHold(button) || totalBatteries(batteries) >= 2 && saysDetonate(button)) {
    ...
    if (totalBatteries(batteries) >= 3 && hasIndicator(indicators, {label: "FRK", isLit: true})) {
        return { description: "Presionar y soltar", action: "tap" }
    }
    return { description: "Mantener y mirar franja", action: "hold" }
}
